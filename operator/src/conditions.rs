...
pub fn upgrade_failed_condition(
    generation: Option<i64>,
    existing_status: &Option<TrustedExecutionClusterStatus>,
    detail: &str,
) -> Condition {
...
mod tests {
    ...
    // random network error message.
    #[test]
    fn test_upgrade_failed_condition_random_network_error_message() {
        let c = upgrade_failed_condition(Some(3), &None, "timeout after 300s");
        assert_eq!(c.type_, UPGRADE_CONDITION);
        assert_eq!(c.reason, UPGRADE_FAILED);
