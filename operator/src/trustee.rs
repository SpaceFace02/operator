...
/// PCR registers compared by tpm.rego. Always emitted so deleting the last
/// ApprovedImage overwrites Trustee with empty arrays instead of leaving
/// stale hashes that would still attest.
...
/// Strategic-merge-patch the full Trustee Deployment spec. Unlike
/// `generate_kbs_deployment` (create-only), this updates an existing
/// deployment in place so the RollingUpdate strategy can keep old pods
/// alive while new pods start.
...
mod tests {
    ...
    // Makes sure the reference values are computed correctly for an empty PCRs, it should have a single value for the SVN.
    #[test]
    ...
    fn test_recompute_reference_values_multiple_images() {
        ...
        let pcrs2 = pcrs_to_status(&vec![other_pcr4, other_pcr14]);
    ...
    async fn test_update_rvs_success() {
        ...
        count_check!(3, clos, |client| {
            // Dummy public_trustee_addr is not a real KBS; sync must fail so
            // callers retry instead of dropping the finalizer with stale hashes.
            assert!(update_reference_values(client).await.is_err());
        });
    }

    #[tokio::test]
    async fn test_update_rvs_no_committed_images() {
        let _ = jsonwebtoken_openssl::install_default();
        let clos = async |req: Request<_>, ctr| match (ctr, req.method()) {
            (0, &Method::GET) => {
    ...
    fn test_recompute_reference_values_includes_svn() {
        let pcrs = vec![dummy_status_pcrs()];
        let result = recompute_reference_values(&pcrs);
        let svn = result.iter().find(|rv| rv.name == "tmp_svn").unwrap();
