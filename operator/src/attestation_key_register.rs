...
/// Ensures the AK secret has the label the secret controller watches on.
/// Secrets created by older operator versions lack it, as older operator versions watched all secrets, without label filters.
                ...
                if tec_deleting {
                    info!(
                        "TrustedExecutionCluster is being deleted, \
                         skipping update for AttestationKey secret {secret_name}"
                    );
                    return Ok(LONG_REQUEUE);
                }
